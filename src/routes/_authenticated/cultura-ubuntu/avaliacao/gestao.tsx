...
function GestaoAvaliacao() {
  ...
  const saveMutation = useMutation({ mutationFn: async () => { if (!selected) return; await update({ data: { id: selected.id, title, blocks: draft ?? loadAvaliacaoDoc(selected.blocks), cover_url: selected.cover_url, cover_position: selected.cover_position ?? undefined, cover_scale: selected.cover_scale ?? undefined } }); }, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["paginas-avaliacao"] }); } });
  const choose = (page: AvaliacaoPage) => { setSelectedId(page.id); setTitle(page.title); setDraft(loadAvaliacaoDoc(page.blocks)); };
  useEffect(() => { if (selected && !draft) { setTitle(selected.title); setDraft(loadAvaliacaoDoc(selected.blocks)); } }, [selected, draft]);
