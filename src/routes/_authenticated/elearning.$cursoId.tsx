      ...
      <div className="mx-auto w-full min-w-0 max-w-[1440px] pb-20 lg:pb-0">
        <section className="sticky top-14 z-20 -mx-4 border-b bg-background/95 shadow-sm backdrop-blur sm:-mx-6 lg:-mx-8">
          ...
          <nav aria-label="Secções do curso" className="flex h-11 items-end gap-6 px-4 sm:px-6 lg:px-8">
            ...
            {curso.inscricao || isPreview ? <Link to="/elearning/$cursoId/passo/$passoId" params={{ cursoId, passoId: formationStepId ?? "" }} disabled={!formationStepId} className={cn("flex h-11 items-center border-b-2 text-sm font-medium", isFormation ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground", !formationStepId && "pointer-events-none opacity-50")}>Formação</Link> : <Tooltip><TooltipTrigger asChild><span className="flex h-11 cursor-not-allowed items-center border-b-2 border-transparent text-sm font-medium text-muted-foreground opacity-50" aria-disabled="true">Formação</span></TooltipTrigger><TooltipContent>Inscreve-te para começar</TooltipContent></Tooltip>}
          </nav>
        </section>
