  ...
  .handler(async ({ data, context }) => {
    ...
    const [actionRes, enrollRes, trainerRes] = await Promise.all([
      supabaseAdmin
        .from("acoes")
        .select(
          "id, title, description, category, action_date, start_date, end_date, registration_status, max_capacity, entity_id, program_id, tshirt_tracking_link, tshirt_value, fotos_link, avaliacao_satisfacao, avaliacao_satisfacao_link, avaliacao_impacto, avaliacao_impacto_link",
        )
        .eq("id", data.actionId)
        .maybeSingle(),
      supabaseAdmin
